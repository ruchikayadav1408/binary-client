...
const generateUserId = (username) => {
  ...
  // return `${cleanedUsername}${uuidPart}`;

  const lettersPart = username.substring(0, 3).toLowerCase();

    // Generate a random 3-digit number
    const numericPart = Math.floor(100 + Math.random() * 900);

    // Combine the letters and numeric parts to create the user ID
    const userId = `${lettersPart}${numericPart}`;
